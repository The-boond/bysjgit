            ...
            <template>
                <div class="createTemplate">
                    <p class="title">{{ formData.id ? '修改' : '录入' }}报销申请</p><a-divider />
                    ...
                    <a-row :gutter="24">
                            ...
                            <a-col :span="12">
                                <a-form-item label="票据图片"  name="billpicture">
                                    <a-upload v-model:file-list="formData.billpicture" name="file" list-type="picture-card"
                                        class="cover-uploader" :show-upload-list="false" action="/api/file/upload" :maxCount="1"
                                        @change="BillpictureUploadChange">
                                        <a-image v-if="billpictureImageUrl" :src="billpictureImageUrl" :preview="false" />
                                        <a-image v-else-if="formData.billpicture" :src="formData.billpicture[0].url" :preview="false" />
                                ...
                                <a-col :span="12">
                                    <a-form-item label="审核"  name="shenhe">
                                        <a-radio-group v-model:value="formData.shenhe" placeholder="请选择审核">
                            ...
                            <a-col :span="12">
                                <a-form-item label="审核回复"  name="shenhehuifu">
                                    <a-textarea v-model:value="formData.shenhehuifu" placeholder="请输入审核回复"
                ...
                const OnSubmit = async (values) => {
                    ...
                    const data = Object.assign(values, {
                        id,
                            ...
                            billpicture: formData.billpicture ? formData.billpicture[0]?.url == undefined ? formData.billpicture[0]?.response?.data?.url : formData.billpicture[0]?.url : undefined,
                        
                    })
                    const { code } = await POST(`/claims/${id != undefined ? "update" : "save"}`, data);
