    ...
    >
        ...
        <template v-if="column.dataIndex === 'opera'">
          <a-space wrap>
            ...
            <a-button
              ...
              @click="InformationssOpen(record)"
              >审核</a-button
            >
          </a-space>
        </template>
      ...
      <template #expandedRowRender="{ record }">
        <a-descriptions bordered>
          <a-descriptions-item label="资料图片" :span="1" v-if="record.photo">
            <a-image
              :width="100"
              :height="100"
              :src="record.photo"
    ...
    >
      ...
      >
        <a-form-item label="审核">
          <a-select ref="select" v-model:value="informationssFormState.shenhe1">
    ...
    >
      <a-form class="formMessage" layout="inline" :model="formMessage">
        <a-row :gutter="24">
          ...
          <a-col :span="12">
            <a-form-item label="资料图片" name="photo">
              <p v-html="formMessage.photo"></p>
            </a-form-item>
          </a-col>
...
const InformationssOk = async () => {
  const { id, shenheid, shenhe1 } = informationssFormState;
...
const MessageRow = async ({ id }) => {
  const { code, data } = await POST("/informationss/info", { id });
  if (code != 0) return;
