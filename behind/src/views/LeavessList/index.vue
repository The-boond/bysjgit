  ...
  <div class="listTemplate" ref="contentRef">
    <div class="topChunk" ref="topChunk">
      <a-form
        ...
        @finish="FinishSearch"
      >
        <a-form-item class="formSearch-item" label="请假人" name="addusername">
          <a-input
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'opera'">
          <a-space wrap>
            ...
            <a-button type="link" @click="LeavessOpen(record)">审核</a-button>
          </a-space>
        </template>
      </template>
      <template #expandedRowRender="{ record }">
        <a-descriptions bordered>
          <a-descriptions-item
            label="请假理由"
            :span="3"
            v-if="record.reasonforleave"
          >
            <p style="margin: 0" v-html="record.reasonforleave"></p>
          </a-descriptions-item>
    ...
    <a-modal v-model:open="leavessOpen" title="审核" @ok="LeavessOk">
      <a-form
        :model="leavessFormState"
        :label-col="{ span: 6 }"
        :wrapper-col="{ span: 16 }"
        autocomplete="off"
      >
        <a-form-item label="审核">
          <a-select ref="select" v-model:value="leavessFormState.shenhe">
    ...
    >
      <a-form class="formMessage" layout="inline" :model="formMessage">
        <a-row :gutter="24">
          <a-col :span="12">
            <a-form-item label="请假人id" name="adduserid">
              <p v-html="formMessage.adduserid"></p>
            </a-form-item>
          </a-col>

          <a-col :span="12">
            <a-form-item label="请假人" name="addusername">
          ...
          <a-col :span="12">
            <a-form-item label="请假理由" name="reasonforleave">
              <p v-html="formMessage.reasonforleave"></p>
            </a-form-item>
          </a-col>

          <a-col :span="12">
            <a-form-item label="请假时间" name="leavetime">
              <p v-html="formMessage.leavetime"></p>
            </a-form-item>
          </a-col>
...
const leavessOpen = ref(false);
const leavessFormState = reactive({});
const LeavessOpen = (row) => {
  leavessOpen.value = true;
  Object.assign(leavessFormState, row);
};
const LeavessOk = async () => {
  const { id, shenheid, shenhe } = leavessFormState;
  const { code } = await POST("/leavess/update", {
    id,
    shenhe,
  });
  leavessOpen.value = false;
  GetData();
};

const DelRow = async ({ id }) => {
  const { code } = await POST("/leavess/delete", [id]);
...
const MessageRow = async ({ id }) => {
  const { code, data } = await POST("/leavess/info", { id });
  if (code != 0) return;
...
const OnSubmit = async (values) => {
  ...
  const { code } = await POST(
    `/leavess/${id != undefined ? "update" : "save"}`,
    data
  );
  if (code != 0) return message.error("创建失败");
  router.push("/LeavessList");
};
// 重置表单
const OnReset = () => {
