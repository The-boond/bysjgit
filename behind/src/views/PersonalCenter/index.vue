    ...
    <div class="PersonalCenter">
                ...
                <a-col :span="24">
                    <a-form-item label="头像">
                        <a-upload class="form-uploader" name="file" action="/api/file/upload" list-type="picture-card"
                            ...
                            <img class="showImg" v-if="imageUrl == ''" :src="formData.avatarurl" alt="avatar" />
        ...
        <a-row :getter="24">
            <a-col :span="4" :offset="10">
                <a-space>
                    <a-button v-if="!editBtnFlag" type="primary" @click="Save()">保存</a-button>
                    <a-button v-if="!editBtnFlag" @click="Reset()">重置</a-button>
                    <a-button v-else type="primary" @click="UpdateConfig(0)">修改</a-button>
                </a-space>
            </a-col>
        </a-row>
    </div>
...
const GetUserInfo = async () => {
    ...
    let resultRes = await POST(`/${useState.GetReqUrl()}/info`, {
        id: userid
    });
    let { code } = resultRes;
    if (code == 0) {
...
const Save = () => {
    formRef.value
        ...
        .then(async () => {
            ...
            let { userid } = useState.$state;
            let resultRes = await POST(`/${useState.GetReqUrl()}/update`, Object.assign(formData, {
                id: userid,
                password: undefined,
            }));
            if (resultRes.code == 0) {
