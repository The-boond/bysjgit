  ...
  <div class="Details_1">
          ...
          <a-row :gutter="[8, 8]">
            <a-col :span="8">
              <p class="content-contentChunk-item">
                <span class="item-key">资料图片:</span>
                <span class="item-value">{{ detailsInfo?.photo }}</span>
              </p>
            </a-col>

            <a-col :span="24">
              <p class="content-contentChunk-item">
                <span class="item-key">资料简介:</span>
                <span class="item-value">{{ detailsInfo?.detail }}</span>
              </p>
            </a-col>
          </a-row>
        </div>
    ...
    <div class="containerBottom">
      <a-tabs v-model:activeKey="activeKey" type="card">
        <a-tab-pane key="0" tab="资料文件">
          <h3>资料文件</h3>
          <div class="single-block" v-if="detailsInfo.shenhe === '同意'">
            <a
              :title="JSON.parse(detailsInfo.files).name"
              :href="
                detailsInfo?.files ? JSON.parse(detailsInfo?.files).url : ''
              "
              >{{ JSON.parse(detailsInfo.files).name }}</a
            >
          </div>
          <div class="single-block" v-else>
            <span>管理员审核通过方可下载</span>
          </div>
        </a-tab-pane>
...
const GetDetails = async () => {
  const { code, data } = await POST("/informationss/info", {
    id: history.state.id,
  });
  if (code != "0") return;
